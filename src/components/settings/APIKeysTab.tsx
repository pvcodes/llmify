...
export function APIKeysTab() {
    ...
    const handleSaveKey = async (provider: ModelProvider) => {
        ...
        setState(prev => ({ ...prev, status: { ...prev.status, [provider]: null } }));
        ...
        if (isValid) {
            ...
            setTimeout(() => setState(prev => ({ ...prev, status: { ...prev.status, [provider]: null } })), 2000);
        } else {
            setState(prev => ({ ...prev, status: { ...prev.status, [provider]: "error" } }));
            setTimeout(() => setState(prev => ({ ...prev, status: { ...prev.status, [provider]: null } })), 3000);
        }
    };
    ...
    const handleCopyKey = async (provider: ModelProvider) => {
        ...
        if (key) {
            ...
            setState(prev => ({ ...prev, status: { ...prev.status, [provider]: "copied" } }));
            setTimeout(() => setState(prev => ({ ...prev, status: { ...prev.status, [provider]: null } })), 2000);
        }
    };
    ...
    return (
        <Card className="border-0 shadow-sm">
            <CardHeader className="pb-2">
                <CardTitle className="text-xl font-semibold tracking-tight">API Keys Configuration</CardTitle>
            ...
            <CardContent className="p-0">
                ...
                {isInitializing ? (
                    <div className="px-6 py-4 text-center text-muted-foreground">Initializing secure key storage...</div>
                ) : cryptoKey ? (
